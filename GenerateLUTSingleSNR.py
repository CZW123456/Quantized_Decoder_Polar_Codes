...
def main():
    ...
    if is_quantize_channel_MMI:
        save_dir = "./LUTs/SingleSNR/N{:d}_ChannelQ{:d}_DecoderQ{:d}_ChannelMMI".format(N, quantize_channel_MMI, quantize_decoder)
        ...
        save_dir = "./LUTs/SingleSNR/N{:d}_ChannelQ{:d}_DecoderQ{:d}_ChannelUniform".format(N, quantize_channel_uniform, quantize_decoder)
    ...
    # begin generating LUT
    EbN0 = 10 ** (design_EbN0 / 10)  # linear scale snr
    sigma = (1 / (2 * rate * EbN0))  # Gaussian noise variance for current EbN0

    print("Eb/N0 = {:0f} dB, Sigma2 = {:6f}".format(design_EbN0, sigma))
    ...
    lut_fs, lut_gs, virtual_channel_llrs = decoder_quantizer.find_quantizer(pzx)

    print("LUT Generation Finished...")

    save_path_f = os.path.join(save_dir, "LUT_F_EbN0dB={:d}.pkl".format(design_EbN0))
    save_path_g = os.path.join(save_dir, "LUT_G_EbN0dB={:d}.pkl".format(design_EbN0))
    save_path_virtual_channel_llr = os.path.join(save_dir, "LLR_EbN0dB={:d}.pkl".format(design_EbN0))

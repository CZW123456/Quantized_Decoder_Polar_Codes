...
class SCLloydDecoder():

    def __init__(self, N, K, frozen_bits, msg_bits, decoder_boundary_f, decoder_boundary_g, decoder_reconstruct_f,
                 decoder_reconstruct_g, v):
        self.N = N
        self.K = K
        self.frozen_bits = frozen_bits
        self.msg_posi = msg_bits
        self.decoder_boundary_f = decoder_boundary_f
        self.decoder_boundary_g = decoder_boundary_g
        self.decoder_reconstruct_f = decoder_reconstruct_f
        self.decoder_reconstruct_g = decoder_reconstruct_g
        self.v = v

    def q_f(self, a, b, boundary, reconstruct):
        result = (1 - 2 * (a < 0)) * (1 - 2 * (b < 0)) * np.min(np.concatenate([np.abs(a), np.abs(b)]).reshape((2, a.shape[0])), axis=0)
        quantized_result = []
        for res in result:
            if res <= boundary[0]:
        ...
        while done == False:
            ...
            if depth == n:
                ...
                if node_state[node_posi] == 0:  # 0 means this node is first achieved in the traversal, calc f value
                    ...
                    boundary = self.decoder_boundary_f[node_posi]
                    reconstruct = self.decoder_reconstruct_f[node_posi]
                    LLR[int(depth), int(temp * node) : int(temp) * int(node + 1)] = self.q_f(a, b, boundary, reconstruct)
                    ...
                    depth += 1
                    temp /= 2
                    boundary = self.decoder_boundary_g[node_posi]
                    reconstruct = self.decoder_reconstruct_g[node_posi]
                    LLR[depth, int(temp) * node : int(temp) * (node + 1)] = self.q_g(a, b, ucapl, boundary, reconstruct)

...
class SCUniformQuantizedDecoder():
    ...
    def __init__(self, N, K, frozen_bits, msg_bits, decoder_r_f, decoder_r_g, v):
        self.N = N
        self.K = K
        self.frozen_bits = frozen_bits
        self.msg_posi = msg_bits
        self.decoder_r_f = decoder_r_f
        self.decoder_r_g = decoder_r_g
        self.v = v

    def q_f(self, a, b, M, r):
        result = (1 - 2 * (a < 0)) * (1 - 2 * (b < 0)) * np.min(np.concatenate([np.abs(a), np.abs(b)]).reshape((2, a.shape[0])), axis=0)
        result[np.abs(result) <= M] = (np.floor(result[np.abs(result) <= M] / r).astype(int) + 1 / 2) * r
        result[np.abs(result) > M] = np.sign(result[np.abs(result) > M]) * (M - r/2)
        return result

    def q_g(self, a, b, c, M, r):
        result = b + (1 - 2 * c) * a
        result[np.abs(result) <= M] = (np.floor(result[np.abs(result) <= M] / r).astype(int) + 1 / 2) * r
        result[np.abs(result) > M] = np.sign(result[np.abs(result) > M]) * (M - r / 2)
        return result
        ...
        while done == False:
            ...
            if depth == n:
                ...
                if node_state[node_posi] == 0:  # 0 means this node is first achieved in the traversal, calc f value
                    ...
                    r_f = self.decoder_r_f[node_posi]
                    M = (self.v//2 - 1) * r_f
                    LLR[int(depth), int(temp * node) : int(temp) * int(node + 1)] = self.q_f(a, b, M, r_f)  # update the LLR in current node
                    ...
                    depth += 1
                    temp /= 2
                    r_g = self.decoder_r_g[node_posi]
                    M = (self.v//2 - 1) * r_g
                    LLR[depth, int(temp) * node : int(temp) * (node + 1)] = self.q_g(a, b, ucapl, M, r_g)

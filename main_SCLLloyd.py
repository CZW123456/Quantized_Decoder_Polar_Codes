...
def main():
    ...
    Q_channel = LloydQuantizer(v, max_iter=200)
    Q_decoder = LLRLloydGA(N, v, max_iter=200)
    ...
    if os.path.isfile(os.path.join(save_dir, "N{:d}_K{:d}_v{:d}".format(N, K, v))):
        ...
        decoder_boundary_f, decoder_boundary_g, decoder_reconstruct_f, decoder_reconstruct_g = Q_decoder.generate_Lloyd_quantizers(sigma)
    ...
    polar_decoder = SCLLloydQuantizedDecoder(N, K, L, frozen_indicator, message_indicator, decoder_boundary_f,
                                             decoder_boundary_g,
    ...
    # save BER/BLER
    ber_save_dir = "simulation_result/BER/SCLLloyd"
    bler_save_dir = "simulation_result/BLER/SCLLloyd"

...
FastSCLLUT::FastSCLLUT(int N_, int K_, int L_, vector<int> frozen_bits_, vector<int> message_bits_,
...
py::array_t<uint8_t> FastSCLLUT::decode(py::array_t<int> &channel_quantized_symbols) {
    ...
    // encoding again
    vector<uint8_t> x(N);
    memcpy(x.data(), ucap[idx].data(), sizeof(uint8_t)*N);
    int m = 1;
    for (int d = n - 1; d >= 0; --d) {

...
CASCLLUT::CASCLLUT(int N_, int K_, int A_, int L_, vector<int> frozen_bits_, vector<int> message_bits_,
                   int crc_n_, vector<int>& crc_loc, vector<vector<vector<vector<int>>>> lut_f_,
...
py::array_t<uint8_t> CASCLLUT::decode(py::array_t<int> &channel_quantized_symbols) {
    ...
    for (int i = 0; i < L; ++i) {
        ...
        for (int k = 0; k < N; ++k) {
            if (frozen_bits[k] == 0){
                decoded_info[cnt] = ucap[sorted_idx[i]][n*N+k];
                ++cnt;
            }
        }

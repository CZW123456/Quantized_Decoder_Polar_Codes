...
def main():
    ...
    mu = args.mu
    rate = K / N
    print("N = {:d}, K = {:d}, L = {:d}, R = {:.2f}, mu = {:d}".format(N, K, L, rate, mu))
    ...
    for EbN0dB in EbN0dBTest:
        ...
        polar_decoder = SCLLUTDecoder(N, K, L, frozen_indicator, message_indicator, fs, gs, virtual_channel_llrs)
        Nbiterrs = 0
        Nblkerrs = 0
        Nblocks = 0
    ...
    # BER / BLER save path
    ber_save_dir = "simulation_result/BER/SCLLUT_DegradMerge"
    bler_save_dir = "simulation_result/BLER/SCLLUT_DegradMerge"
    ...
    ber_save_path = os.path.join(ber_save_dir, "N{:d}_K{:d}_L{:d}_Mu{:d}".format(N, K, L, mu))
    bler_save_path = os.path.join(bler_save_dir, "N{:d}_K{:d}_L{:d}_mu{:d}".format(N, K, L, mu))

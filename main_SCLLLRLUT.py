...
def main():
    ...
    for EbN0dB in EbN0dBTest:
        ...
        for i in range(int(quantize_channel)):
            ...
            quanta_channel[i] = np.sum(quanta_channel_uniform[begin:end] * pxz) / np.sum(
                pxz)  # the quanta should be the mass center of the quantization interval
        ...
        polar_decoder = SCLLUTDecoder(N, K, L, frozenbits_indicator, messagebits_indicator, fs, gs, llr_quanta)
    ...
    # BER / BLER save path
    ber_save_dir = "simulation_result/BER/SCLLLRLUT"
    bler_save_dir = "simulation_result/BLER/SCLLLRLUT"

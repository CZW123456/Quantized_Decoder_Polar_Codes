...
def main():
    ...
    if is_quantize_channel_MMI:
        load_dir = "./LUTs/N{:d}_K{:d}_ChannelQ{:d}_DecoderQ{:d}_ChannelMMI".format(N, K, quantize_channel_MMI,
                                                                                    quantize_decoder)
        print(
            "channel uniform quantization level = {:d}\nchannel MMI quantization level = {:d}\ndecoder quantization level = {:d}".format(
                quantize_channel_uniform, quantize_channel_MMI, quantize_decoder))
    else:
    ...
    # BER / BLER save path
    ber_save_dir = "simulation_result/BER/SCLUT"
    bler_save_dir = "simulation_result/BLER/SCLUT"
    ...
    if is_quantize_channel_MMI:
        ber_save_path = os.path.join(ber_save_dir, "N{:d}_K{:d}_ChannelQ{:d}_DecoderQ{:d}_ChannelMMI".format(N, K,
                                                                                                             quantize_channel_MMI,
                                                                                                             quantize_decoder))
        bler_save_path = os.path.join(bler_save_dir, "N{:d}_K{:d}_ChannelQ{:d}_DecoderQ{:d}_ChannelMMI".format(N, K,
                                                                                                               quantize_channel_MMI,
                                                                                                               quantize_decoder))
    else:
        ber_save_path = os.path.join(ber_save_dir,
                                     "N{:d}_K{:d}_ChannelQ{:d}_DecoderQ{:d}".format(N, K, quantize_channel_MMI,
                                                                                    quantize_decoder))
        bler_save_path = os.path.join(bler_save_dir,
                                      "N{:d}_K{:d}_ChannelQ{:d}_DecoderQ{:d}".format(N, K, quantize_channel_MMI,
                                                                                     quantize_decoder))
